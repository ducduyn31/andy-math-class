...
export const DarkModeToggle: React.FC<Props> = ({}) => {
  const { isDarkMode: isThemeReverted, toggle } = useDarkMode();
  ...
  return (
    <div>
      <input
        ...
        value={isDarkModePreferred ? "corporate" : "dim"}
        checked={isThemeReverted}
        onChange={(e) => {
          e.preventDefault();
        }}
      />
      <label className="swap swap-rotate" onClick={toggle}>
        <svg
          className={
            isDarkMode
              ? "swap-off fill-current w-10 h-10"
